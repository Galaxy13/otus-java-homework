...
public class Animal {
    ...
    private double exhaustionCheck(double distance, double velocity, int enduranceCost) {
        if (endurance < 0) {
            System.out.printf("%s is exhausted. %s meters not finished%n", animalType, -endurance / enduranceCost);
            endurance = 0;
            return -1;
        } else {
            System.out.printf("%s successfully finished. Endurance left %s%n", animalType, endurance / enduranceCost);
            return velocity * distance;
        }
    }

    public void info() {
        System.out.printf("%nAnimal type: %s%n", animalType);
